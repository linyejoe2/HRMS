...
```
{
  ...
  agent?: string; // empID of the HR/admin who created this request on the employee's behalf (hrMode / 代理申請); the request still goes through the normal approval workflow
...
- **Approval Workflow** (leave, business-trip, post-clock, official-business):
  ...
  - Leave's 狀態 chip (`AskLeaveTab.tsx`/`ApproveLeaveList.tsx`) shows the current stage — 代理人審核中 / 代理人拒絕 / 主管審核中 / 主管拒絕 / 待審核 (HR) / 已核准 / 人事直接核准 (HR overrode before both stages cleared) — and is clickable to open a stage-by-stage `ApprovalTimelineModal` (reusable for other request types); a rejected substitute/manager stage moves the request into `ApproveLeaveList.tsx`'s 已拒絕 sub-tab even though the backend `status` field stays `created` until HR's final decision
  ...
  - Business-trip (`BusinessTripTab.tsx`/`ApproveBusinessTripList.tsx`) and official-business (`OfficialBusinessTab.tsx`/`ApproveOfficialBusinessTab.tsx`) 狀態 chips follow the single-review-stage version of the same pattern — 主管審核中 / 主管拒絕 / 待審核 (HR) / 已核准 / 人事直接核准 / 已拒絕 / 已取消 — also clickable to open the shared `ApprovalTimelineModal` (建立→主管簽核→人事審核)
