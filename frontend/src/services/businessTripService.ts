import { BusinessTripRequest, ApproveStatus } from '../types';
...
// ApprovalTimelineModal, from a business-trip request's manager/status fields.
// `names` resolves empID -> employee name for the manager/HR actors (the requester's
// own name is already on the request). `manager` is only set once the manager
// actually acts, so `fallbackManagerEmpID` (the requester's assigned Employee.manager)
// is used to still show who is expected to sign while pending.
export function getBusinessTripApprovalStages(
