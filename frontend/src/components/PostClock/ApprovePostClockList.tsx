...
import { getAllPostClockRequests, approvePostClockRequest, rejectPostClockRequest, cancelPostClockRequest, postClockAPI } from '../../services/api';
...
const ApprovePostClockList: React.FC = () => {
  ...
  const [attachmentRequest, setAttachmentRequest] = useState<PostClockRequest | null>(null);
  ...
  const updatePostClockRequest = (updatedRequest: PostClockRequest) => {
    setPostClockRequests(requests => requests.map(request => request._id === updatedRequest._id ? updatedRequest : request));
  ...
  const handleAttachmentUpload = async (files: File[]) => {
    ...
    try {
      const response = await postClockAPI.uploadAttachments(attachmentRequest._id, files);
      updatePostClockRequest(response.data.data);
      toast.success('附件已新增');
    } catch (error: any) {
  ...
  const handleAttachmentDelete = async (filePath: string) => {
    if (!attachmentRequest?._id) return;
    try {
      const response = await postClockAPI.deleteAttachment(attachmentRequest._id, filePath);
      updatePostClockRequest(response.data.data);
      toast.success('附件已刪除');
    } catch (error: any) {
